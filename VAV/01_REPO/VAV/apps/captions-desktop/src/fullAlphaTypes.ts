import type {AbraxasArtifact, CaptionStylePreset, MotionPreset} from "@vav/abraxas-import";
import type {VisionProviderReport} from "@vav/vision-contracts";

export type RuntimeCaption = Readonly<{
  id: string;
  startUs: number;
  endUs: number;
  text: string;
...
export type AbraxasImportRecord = Readonly<{
  importId: string;
  sourcePath: string;
  importedAt: string;
  artifact: AbraxasArtifact;
}>;
...
export type SavedProject = Readonly<{
  ...
  abraxasImports?: readonly AbraxasImportRecord[];
}>;

export type {AbraxasArtifact, CaptionStylePreset, MotionPreset, VisionProviderReport};
