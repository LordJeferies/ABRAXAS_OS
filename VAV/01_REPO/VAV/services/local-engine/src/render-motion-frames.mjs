...
const composition = await selectComposition({
  ...
  inputProps: {motionPlan, editLock}
});
...
const summary = {
  status: allExist && uniqueHashCount >= 3 ? "PASS" : "FAIL",
...
fs.writeFileSync(path.join(outDir, "render_evidence.json"), JSON.stringify(summary, null, 2));
