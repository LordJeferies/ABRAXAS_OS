...
function generateLunaComercialPage(locale) {
  ...
  const quickItems = [
    ...
    { label: '🎯 Ventas por Campaña', href: '#ventas-campana' },
    { label: '🔗 Ventas por Pieza', href: '#atribucion' },
...
<body>
  ...
  <main class="section-wrap" style="padding-top: 50px;">
    ...
    <!-- SECCIÓN: ATRIBUCIÓN JERÁRQUICA: CAMPAÑAS, PIEZAS Y CATÁLOGO DE PRODUCTOS -->
    <section id="atribucion-jerarquica" style="margin-bottom: 4rem;">
      <div class="section-head" style="text-align: left; margin-bottom: 2rem;">
        <span class="tag" style="color: #d4af37;">TOPOLOGÍA N:M // ATRIBUCIÓN DE NEGOCIO REAL</span>
        <h2 class="h2" style="font-size: 2rem;">${isEs ? 'Atribución Dual: Por Campaña Global, Por Pieza y Por Producto' : 'Dual Attribution: By Campaign, By Asset & By Product'}</h2>
        <p class="p" style="margin: 0; font-size: 1rem;">${isEs ? 'Audita tanto campañas multioferta (1 campaña = varios productos) como lanzamientos dedicados (1 producto = 15 piezas), desglosando el ROI en cualquier dimensión.' : 'Audits both multi-offer campaigns (1 campaign = multiple products) and dedicated launches, breaking down ROI in any business dimension.'}</p>
      </div>

      <div class="bento-grid">
        <div class="spotlight-card col-4" style="background: #090a10; border-top: 3px solid #d4af37;">
          <span class="card-pill-tag gold">VISTA A // POR CAMPAÑA</span>
          <h3 class="card-h3">${isEs ? 'Rentabilidad Macro de Campaña' : 'Macro Campaign ROI'}</h3>
          <p class="card-desc" style="font-size: 0.88rem; color: #cbd5e1; line-height: 1.55;">
            ${isEs 
              ? 'Agrupa todas las piezas y productos involucrados. Mide el costo total de manufactura del lote vs. la facturación acumulada de todas las ofertas vendidas.' 
              : 'Aggregates all published assets and products. Computes total manufacturing cost vs total sales generated by the campaign.'}
          </p>
        </div>

        <div class="spotlight-card col-4" style="background: #090a10; border-top: 3px solid #2997ff;">
          <span class="card-pill-tag cyan">VISTA B // POR PIEZA INDIVIDUAL</span>
          <h3 class="card-h3">${isEs ? 'Auditoría Forense por Activo' : 'Per-Asset Forensic Audit'}</h3>
          <p class="card-desc" style="font-size: 0.88rem; color: #cbd5e1; line-height: 1.55;">
            ${isEs 
              ? 'Identifica qué reel o carrusel exacto disparó la conversación o el clic inicial en WhatsApp, calculando el retorno directo de esa pieza única.' 
              : 'Pinpoints which exact reel or carousel triggered the inbound lead, calculating the direct return of that specific piece.'}
          </p>
        </div>

        <div class="spotlight-card col-4" style="background: #090a10; border-top: 3px solid #30d158;">
          <span class="card-pill-tag emerald">VISTA C // POR CATÁLOGO DE PRODUCTO</span>
          <h3 class="card-h3">${isEs ? 'Tracción por Oferta / SKU' : 'SKU & Offer Traction'}</h3>
          <p class="card-desc" style="font-size: 0.88rem; color: #cbd5e1; line-height: 1.55;">
            ${isEs 
              ? 'Comprueba qué producto se vende más a través del ecosistema de contenidos y qué closer es más efectivo cerrando cada servicio específico.' 
              : 'Tracks which product converts best through content and which closer is most effective closing each specific offer.'}
          </p>
        </div>
      </div>
    </section>
    ...
    <!-- SECCIÓN DEDICADA: VENTAS POR CAMPAÑA GENERAL (MACRO LEDGER) -->
    <section id="ventas-campana" style="margin-bottom: 4rem;">
      <div class="section-head" style="text-align: left; margin-bottom: 2rem;">
        <span class="tag" style="color: #f59e0b;">01 // REPORTE MACRO DE VENTAS POR CAMPAÑA</span>
        <h2 class="h2" style="font-size: 2rem;">${isEs ? 'El Registro de Ventas por Campaña General' : 'Macro Campaign Sales Ledger'}</h2>
        <p class="p" style="margin: 0; font-size: 1rem;">${isEs ? 'Auditoría agregada de campañas completas. Evalúa el rendimiento comercial de lotes enteros de 10 a 50 piezas que impulsan uno o varios productos simultáneamente.' : 'Aggregated audit of full campaigns. Evaluates commercial returns of 10-50 asset batches promoting single or multiple products simultaneously.'}</p>
      </div>

      <div class="spotlight-card col-12" style="position: relative; border-left: 4px solid #f59e0b; margin-bottom: 1.5rem;">
        <button class="card-expand-btn" title="Expandir">+</button>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 10px;">
          <div>
            <span class="card-pill-tag gold">TABLERO RELACIONAL SQLITE // CAMPAIGNS & PRODUCTS</span>
            <h3 class="card-h3" style="margin: 4px 0 0 0;">${isEs ? 'Consolidado Financiero por Campaña Activa' : 'Financial Summary by Active Campaign'}</h3>
          </div>
          <span style="font-family: var(--font-mono); font-size: 0.8rem; color: #94a3b8;">Tabla: <code>campaigns</code> ⨝ <code>sales_transactions</code></span>
        </div>

        <div style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-family: var(--font-mono); font-size: 0.85rem; text-align: left;">
            <thead>
              <tr style="border-bottom: 1px solid rgba(255,255,255,0.15); color: #f59e0b;">
                <th style="padding: 12px 14px;">CAMPAÑA ID / NOMBRE</th>
                <th style="padding: 12px 14px;">OBJETIVO</th>
                <th style="padding: 12px 14px;">PIEZAS EN LOTE</th>
                <th style="padding: 12px 14px;">PRODUCTOS / SKUs</th>
                <th style="padding: 12px 14px;">COSTO MANUFACTURA</th>
                <th style="padding: 12px 14px;">VENTAS COBRADAS (OCR)</th>
                <th style="padding: 12px 14px; text-align: right;">ROI CAMPAÑA</th>
              </tr>
            </thead>
            <tbody>
              <tr style="border-bottom: 1px solid rgba(255,255,255,0.06); background: rgba(245, 158, 11, 0.03);">
                <td style="padding: 14px; font-weight: 700; color: #fff;">
                  <code>CAMP_01 // Lanzamiento Ecosistema Q3</code><br/>
                  <span style="font-size: 0.75rem; color: #94a3b8; font-weight: normal;">15 Ago – 01 Sep 2026</span>
                </td>
                <td style="padding: 14px;"><span class="badge" style="background: rgba(56, 189, 248, 0.15); color: #38bdf8;">CONVERSIÓN DIRECTA</span></td>
                <td style="padding: 14px; color: #cbd5e1;">16 piezas (10 reels, 4 carruseles, 2 hilos)</td>
                <td style="padding: 14px; color: #e2e8f0;">Software Core ($1,000) + Membresía ($150)</td>
                <td style="padding: 14px; color: #ef4444; font-weight: 700;">$680.00 USD</td>
                <td style="padding: 14px; color: #30d158; font-weight: 700; font-size: 0.95rem;">$13,450.00 USD</td>
                <td style="padding: 14px; text-align: right; color: #30d158; font-weight: 800; font-size: 1rem;">+1,877%</td>
              </tr>
              <tr style="border-bottom: 1px solid rgba(255,255,255,0.06);">
                <td style="padding: 14px; font-weight: 700; color: #fff;">
                  <code>CAMP_02 // Destrucción de Dogmas IA</code><br/>
                  <span style="font-size: 0.75rem; color: #94a3b8; font-weight: normal;">01 Ago – 15 Ago 2026</span>
                </td>
                <td style="padding: 14px;"><span class="badge" style="background: rgba(168, 85, 247, 0.15); color: #c084fc;">AUTORIDAD & HIGH-TICKET</span></td>
                <td style="padding: 14px; color: #cbd5e1;">8 piezas (6 reels, 2 PDFs LinkedIn)</td>
                <td style="padding: 14px; color: #e2e8f0;">Implementación VIP ($2,500)</td>
                <td style="padding: 14px; color: #ef4444; font-weight: 700;">$352.00 USD</td>
                <td style="padding: 14px; color: #30d158; font-weight: 700; font-size: 0.95rem;">$7,500.00 USD</td>
                <td style="padding: 14px; text-align: right; color: #30d158; font-weight: 800; font-size: 1rem;">+2,030%</td>
              </tr>
              <tr style="border-bottom: 1px solid rgba(255,255,255,0.06); background: rgba(56, 189, 248, 0.02);">
                <td style="padding: 14px; font-weight: 700; color: #fff;">
                  <code>CAMP_03 // Caso Real Amanda & Joc</code><br/>
                  <span style="font-size: 0.75rem; color: #94a3b8; font-weight: normal;">10 Jul – 25 Jul 2026</span>
                </td>
                <td style="padding: 14px;"><span class="badge" style="background: rgba(34, 197, 94, 0.15); color: #4ade80;">PRUEBA DE MECANISMO</span></td>
                <td style="padding: 14px; color: #cbd5e1;">4 piezas (2 reels 18s, 2 carruseles 4:5)</td>
                <td style="padding: 14px; color: #e2e8f0;">Licencia ABRAXAS v3.3 Pro ($4,990)</td>
                <td style="padding: 14px; color: #ef4444; font-weight: 700;">$176.00 USD</td>
                <td style="padding: 14px; color: #30d158; font-weight: 700; font-size: 0.95rem;">$4,990.00 USD</td>
                <td style="padding: 14px; text-align: right; color: #30d158; font-weight: 800; font-size: 1rem;">+2,735%</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p style="font-size: 0.8rem; color: #94a3b8; margin-top: 12px; line-height: 1.5;">
          💡 <strong>Regla de Atribución Macro:</strong> Si una venta en WhatsApp o llamada con closer no puede atribuirse a un video en particular (porque el comprador vio múltiples piezas del lote), se imputa directamente al <code>campaign_id</code> correspondiente, garantizando que el ROI global de la campaña nunca sufra fugas de datos.
        </p>
      </div>
    </section>

    <!-- 2. Vínculo Pieza-Venta (Content Revenue Ledger) -->
    <section id="atribucion" style="margin-bottom: 4rem;">
      <div class="section-head" style="text-align: left; margin-bottom: 2rem;">
        <span class="tag" style="color: #38bdf8;">02 // REGISTRO DE ATRIBUCIÓN POR PIEZA INDIVIDUAL</span>
...
<body>
  ...
  <main class="section-wrap" style="padding-top: 50px;">
    ...
    <section id="campanas" style="margin-bottom: 4rem;">
      <div class="section-head" style="text-align: left; margin-bottom: 2rem;">
        <span class="tag" style="color: #f59e0b;">02 // SISTEMA DE LOTES & MODELOS DE CAMPAÑA</span>
        <h2 class="h2" style="font-size: 2rem;">${isEs ? 'Las 4 Campañas Maestras de Alta Conversión' : 'The 4 Master High-Conversion Campaigns'}</h2>
        <p class="p" style="margin: 0; font-size: 1rem;">${isEs ? 'Cada campaña es un lote coordinado de 10 a 50 piezas de contenido (videos de 18s, carruseles 4:5, hilos de X y PDFs de LinkedIn) diseñadas para llevar a un prospecto desde el desconocimiento total hasta la factura cobrada en WhatsApp o checkout.' : 'Each campaign is a coordinated batch of 10-50 assets designed to take a stranger from unaware to a closed deal in WhatsApp or checkout.'}</p>
      </div>

      <!-- Diagrama de Flujo de Campaña -->
      <div class="spotlight-card col-12" style="background: #090a14; border-top: 3px solid #d4af37; margin-bottom: 2.5rem;">
        <span class="card-pill-tag gold">${isEs ? 'LA ARQUITECTURA DE UNA CAMPAÑA ABRAXAS' : 'ABRAXAS CAMPAIGN ARCHITECTURE'}</span>
        <h3 class="card-h3" style="margin-top: 8px;">${isEs ? '¿Qué contiene exactamente una Campaña Completa?' : 'What does a complete campaign contain?'}</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 16px; margin-top: 14px; font-size: 0.88rem; color: #cbd5e1;">
          <div style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 8px; border-left: 3px solid #38bdf8;">
            <strong style="color: #fff; display: block; margin-bottom: 6px;">1. Tesis & Ángulo Central:</strong>
            <p style="color: #94a3b8; margin: 0;">Un axioma validado por YOD que desafía al mercado y captura el 100% de la atención del nicho.</p>
          </div>
          <div style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 8px; border-left: 3px solid #a855f7;">
            <strong style="color: #fff; display: block; margin-bottom: 6px;">2. Lote de 10 a 50 Activos:</strong>
            <p style="color: #94a3b8; margin: 0;">Reels verticales en 18s, carruseles multicapa Paper-Cut 4:5, ensayos y micro-podcasts bajo el mismo hash Merkle-DAG.</p>
          </div>
          <div style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 8px; border-left: 3px solid #30d158;">
            <strong style="color: #fff; display: block; margin-bottom: 6px;">3. Catálogo de Ofertas (1..N):</strong>
            <p style="color: #94a3b8; margin: 0;">Los productos específicos vinculados (desde productos de entrada hasta implementaciones High-Ticket de $2,500+).</p>
          </div>
          <div style="background: rgba(255,255,255,0.03); padding: 14px; border-radius: 8px; border-left: 3px solid #f59e0b;">
            <strong style="color: #fff; display: block; margin-bottom: 6px;">4. Guiones para Closers:</strong>
            <p style="color: #94a3b8; margin: 0;">Argumentos de venta para que el equipo comercial responda en WhatsApp con el mismo tono y autoridad del video.</p>
          </div>
        </div>
      </div>
      ...
      <div class="bento-grid">
        <div class="spotlight-card col-6" style="position: relative; border-left: 4px solid #d4af37;">
          <span class="card-pill-tag gold">CAMPAÑA 01 // AUTORIDAD & POSICIONAMIENTO</span>
          <h4 class="card-h3">${isEs ? 'Tesis Contraria & Dominio de Territorio' : 'Contrary Thesis & Category Dominance'}</h4>
          <p class="card-desc">${isEs ? 'Ataca las creencias limitantes y mitos de tu industria. Posiciona al fundador como la voz líder indiscutible mediante videos quirúrgicos de 18s y carruseles 4:5 con axiomas de autoridad.' : 'Dismantles industry misconceptions. Positions founder as uncontested category leader via 18s videos and 4:5 carousels.'}</p>
          <ul style="color: #94a3b8; font-size: 0.82rem; margin-top: 10px; line-height: 1.5; padding-left: 1.2rem;">
            <li><strong>Lote típico:</strong> 12 videos de 18s + 4 carruseles + 2 ensayos.</li>
            <li><strong>Objetivo:</strong> Eliminar la resistencia por precio y atraer clientes sofisticados.</li>
          </ul>
        </div>

        <div class="spotlight-card col-6" style="position: relative; border-left: 4px solid #c084fc;">
          <span class="card-pill-tag purple">CAMPAÑA 02 // DESTRUCCIÓN DE DOGMAS</span>
          <h4 class="card-h3">${isEs ? 'Desmantelamiento de Métodos Obsoletos' : 'Dismantling Obsolete Competitor Methods'}</h4>
          <p class="card-desc">${isEs ? 'Expone con pruebas tangibles por qué las soluciones tradicionales de tus competidores ya no funcionan, preparando psicológicamente al comprador para tu producto.' : 'Proves with hard evidence why legacy alternatives fail, conditioning buyers for your mechanism.'}</p>
          <ul style="color: #94a3b8; font-size: 0.82rem; margin-top: 10px; line-height: 1.5; padding-left: 1.2rem;">
            <li><strong>Lote típico:</strong> 8 videos 18s + 4 hilos en X + 2 PDFs LinkedIn.</li>
            <li><strong>Objetivo:</strong> Provocar la duda dialéctica y descalificar a la competencia.</li>
          </ul>
        </div>

        <div class="spotlight-card col-6" style="position: relative; border-left: 4px solid #38bdf8;">
          <span class="card-pill-tag cyan">CAMPAÑA 03 // DEMOSTRACIÓN DE MECANISMO ÚNICO</span>
          <h4 class="card-h3">${isEs ? 'La Prueba Física Irrefutable (Casos Reales)' : 'Irrefutable Proof & Mechanism Walkthrough'}</h4>
          <p class="card-desc">${isEs ? 'Muestra exactamente cómo funciona tu sistema por dentro bajo la regla de las 3 imágenes de evidencia física (START ➔ DEVELOPMENT ➔ RESULT), eliminando el escepticismo técnico.' : 'Shows exactly how your system works using the 3 real evidence images rule (START ➔ DEVELOPMENT ➔ RESULT).'}</p>
          <ul style="color: #94a3b8; font-size: 0.82rem; margin-top: 10px; line-height: 1.5; padding-left: 1.2rem;">
            <li><strong>Lote típico:</strong> 10 videos 18s con Motion M2/M4 + 1 documental 16:9 en YouTube.</li>
            <li><strong>Objetivo:</strong> Demostrar que tu resultado es reproducible y matemáticamente sólido.</li>
          </ul>
        </div>

        <div class="spotlight-card col-6" style="position: relative; border-left: 4px solid #30d158;">
          <span class="card-pill-tag emerald">CAMPAÑA 04 // OFERTA & CONVERSIÓN DIRECTA</span>
          <h4 class="card-h3">${isEs ? 'Cierre en WhatsApp, Pasarelas & Closers' : 'Direct Close in WhatsApp & Checkouts'}</h4>
          <p class="card-desc">${isEs ? 'Secuencia cronometrada que canaliza la autoridad acumulada hacia un llamado a la acción específico: compra directa o llamada de cierre con asesores comerciales.' : 'Timed sequence directing authority into conversion channels: WhatsApp flows, sales calls, and checkouts.'}</p>
          <ul style="color: #94a3b8; font-size: 0.82rem; margin-top: 10px; line-height: 1.5; padding-left: 1.2rem;">
            <li><strong>Lote típico:</strong> 6 videos 18s de oferta directa + plantillas de chat para closers.</li>
            <li><strong>Objetivo:</strong> Maximizar facturación inmediata y retorno contable medido en Luna 3.</li>
          </ul>
        </div>
      </div>
    </section>
