...
from .core import balanced_fragment_ranges, ensure_dir, format_timecode, parse_timecode, read_json, source_fingerprint, stable_hash, write_json_atomic
from .davinci import build_handoff_manifest
from .htmlio import compile_html_pair
from .html_packages import build_html_video_packages
from .microtrim_runner import resolve_microtrim_job
from .motions import build_motion_previews, compile_motion_plan, refresh_asset_status, verify_motion_plan, write_motion_tree
...
def hydrate_cfg(cfg):
    ...
    paths={
      ...
      'visual_queue':str(out/'09_VISUAL_QUEUE'),'motions':str(out/'09_MOTIONS_V4_2'),'davinci':str(out/'10_DAVINCI_HANDOFF'),'final':str(out/'11_FINAL'),
...
def motion_plan_path(cfg): return Path(cfg['_paths']['manifest'])/'MOTION_PLAN_V4_2.json'
...
def cmd_setup(args):
    ...
    if args.joc55:
        ...
        masters=Path.home()/"Desktop/Joc podcast next ep 55/00_MASTERS_OFICIALES"
        dv=str(masters/"JOC55_MASTER_VERTICAL_OFICIAL.mp4")
        dh=str(masters/"JOC55_MASTER_HORIZONTAL_OFICIAL.mp4")
        dout=str(Path.home()/"Desktop/Joc podcast next ep 55/JOC55_ABRAXAS_V4_2_OUTPUT")
        pid='JOC55_AMANDA'
    else:
        content=args.content or ''; intro=args.intro or ''; dv=args.vertical or ''; dh=args.horizontal or ''; dout=args.output or str(Path.home()/'Desktop/ABRAXAS_V4_2_OUTPUT'); pid=args.project_id or 'PROJECT'
...
def cmd_set_microtrim(args):
    ...
    if e<=s or e-s<4.0 or e-s>9.0: raise SystemExit('Manual delivery part must be between 4.000 and 9.000 seconds')
    res[args.beat_id]={'beat_id':args.beat_id,'valid':True,'start':s,'end':e,'start_tc':format_timecode(s),'end_tc':format_timecode(e),'method':'HUMAN_MANUAL_APPROVAL','duration_gate_4_9':'PASS'}; save_resolutions(cfg,res); print('SAVED',args.beat_id)
...
def cmd_plan(args):
    ...
    if cfg.get('motion',{}).get('enabled',True):
        motion_plan=compile_motion_plan(b,plan,res,cfg.get('motion',{}).get('selection_overrides',{}))
        write_motion_tree(motion_plan,cfg['_paths']['root'])
    ...
    if cfg.get('motion',{}).get('enabled',True): summary['motions_v4_2']=motion_plan['counts']
...
def cmd_motions(args):
    cfg=load_cfg(args.config); bundle=load_bundle(cfg); plan=read_json(part_plan_path(cfg)); resolutions=load_resolutions(cfg)
    if not plan: raise SystemExit('PART_PLAN missing; run 08_BUILD_PART_PLAN.command first')
    motion_plan=compile_motion_plan(bundle,plan,resolutions,cfg.get('motion',{}).get('selection_overrides',{}))
    target=write_motion_tree(motion_plan,cfg['_paths']['root'])
    print('MOTION PLAN:',target)
    print(json.dumps(motion_plan['counts'],ensure_ascii=False,indent=2))

def cmd_motion_previews(args):
    cfg=load_cfg(args.config); plan=read_json(motion_plan_path(cfg))
    if not plan: raise SystemExit('MOTION_PLAN_V4_2 missing; run 08B_BUILD_MOTIONS_AND_PROMPTS.command first')
    report=build_motion_previews(cfg,plan,dry_run=args.dry_run)
    write_json_atomic(motion_plan_path(cfg),report['plan'])
    write_json_atomic(Path(cfg['_paths']['manifest'])/'MOTION_PREVIEW_REPORT_V4_2.json',{k:v for k,v in report.items() if k!='plan'})
    counts={}
    for item in report['results']: counts[item['status']]=counts.get(item['status'],0)+1
    print(json.dumps(counts,ensure_ascii=False,indent=2))

def cmd_motion_verify(args):
    cfg=load_cfg(args.config); plan=read_json(motion_plan_path(cfg))
    if not plan: raise SystemExit('MOTION_PLAN_V4_2 missing')
    refresh_asset_status(plan); write_json_atomic(motion_plan_path(cfg),plan)
    report=verify_motion_plan(plan,strict_assets=args.strict_assets)
    write_json_atomic(Path(cfg['_paths']['manifest'])/'MOTION_VERIFY_REPORT_V4_2.json',report)
    print('MOTION VERIFY:',report['status'],'· BLOCKERS:',len(report['blockers']),'· WARNINGS:',len(report['warnings']))
    for item in report['blockers']: print('BLOCKED',item)
    for item in report['warnings'][:30]: print('WARNING',item)
    if report['blockers']: raise SystemExit(2)

def cmd_html_packages(args):
    cfg=load_cfg(args.config); bundle=load_bundle(cfg); resolutions=load_resolutions(cfg); motion_plan=read_json(motion_plan_path(cfg),{}) or {}
    report=build_html_video_packages(
        cfg,
        bundle,
        resolutions,
        render_cuts=not args.structure_only,
        dry_run=args.dry_run,
        motion_plan=motion_plan,
    )
    print('HTML VIDEO PACKAGES V4.2')
    print(json.dumps(report['counts'],ensure_ascii=False,indent=2))
    print('PENDING MICROTRIMS:',len(report['pending_microtrims']))
    for beat_id in report['pending_microtrims'][:100]: print('PENDING',beat_id)
    print('REPORT:',Path(cfg['_paths']['manifest'])/'HTML_VIDEO_PACKAGES_REPORT_V4_2.json')

def _cache_target(cfg,ori,key): return Path(cfg['_paths']['cache'])/'PARTS'/ori.upper()/f'{key}.mp4'

def _render_balanced_fragment_set(cfg, assembled_path, total_seconds, folder, tag, dry_run=False):
    ranges=balanced_fragment_ranges(total_seconds,minimum=4.0,maximum=9.0,target=8.0)
    folder=ensure_dir(folder); results=[]; manifest=[]
    for index,(start,end) in enumerate(ranges,1):
        target=folder/f'{tag}_FRAG_{index:03d}.mp4'
        meta={'cache_key':stable_hash({'assembled':str(assembled_path),'tag':tag,'start':start,'end':end,'output_version':cfg.get('output_version')}),'fragment_id':f'{tag}_FRAG_{index:03d}','timeline_in':start,'timeline_out':end,'duration':round(end-start,3),'contract':'4<=duration<=9'}
        results.append(encode_part(cfg,assembled_path,start,end,target,meta,dry_run=dry_run))
        manifest.append({'fragment_id':meta['fragment_id'],'path':str(target),'timeline_in':start,'timeline_out':end,'duration':meta['duration']})
    write_json_atomic(folder/'FRAGMENT_MANIFEST.json',{'schema_version':'abraxas.fragments.v4.2','total_seconds':round(float(total_seconds),3),'average_seconds':round(sum(x['duration'] for x in manifest)/len(manifest),3),'minimum_seconds':min(x['duration'] for x in manifest),'maximum_seconds':max(x['duration'] for x in manifest),'fragments':manifest})
    return results
...
def _render_intro(cfg,item,sourcefps,dry_run=False):
    ...
    for ori in ('vertical','horizontal'):
        ...
        for variant,ids in item['assemblies'].items():
            ...
            if files:
                out.append(assemble(cfg,files,target,{'content_id':item['content_id'],'orientation':ori,'variant':variant},dry_run=dry_run))
                total=sum(parse_timecode(item['parts'][beat_id]['end'])-parse_timecode(item['parts'][beat_id]['start']) for beat_id in ids if beat_id in item['parts'])
                out.extend(_render_balanced_fragment_set(cfg,target,total,media/'FRAGMENTS'/variant,f"{item['content_id']}_{ori.upper()}_{variant}",dry_run=dry_run))
...
def _render_content(cfg,item,sourcefps,kind,dry_run=False):
    ...
    total=sum(parse_timecode(seg['end'])-parse_timecode(seg['start']) for seg in item['segments'])
    if ori == 'vertical':
...
def cmd_davinci(args):
    ...
    motion_plan=read_json(motion_plan_path(cfg),{}) or {}
    refresh_asset_status(motion_plan)
    write_json_atomic(motion_plan_path(cfg),motion_plan)
    motion_items=motion_plan.get('placements',[])
    revision=cfg.get('davinci',{}).get('revision','V4_2')
    def motions_for(content_id,orientation,variant):
        return [x for x in motion_items if x.get('content_id')==content_id and x.get('orientation')==orientation.lower() and x.get('assembly_variant')==variant]
    def fragment_paths(folder):
        manifest=read_json(Path(folder)/'FRAGMENT_MANIFEST.json',{}) or {}
        return [x['path'] for x in manifest.get('fragments',[]) if Path(x.get('path','')).is_file()]
    resolved=[]
    ...
    for item in plan.get('intros',[]):
        ...
        for ori in ('VERTICAL','HORIZONTAL'):
            ...
            for variant,ids in item.get('assemblies',{}).items():
                paths=fragment_paths(media/'FRAGMENTS'/variant) or [pmap[x] for x in ids if x in pmap and Path(pmap[x]).is_file()]
                if paths:
                    resolved.append({'timeline_name':f"{item['content_id']}_{ori}_{variant}_ABRAXAS_{revision}",'content_id':item['content_id'],'type':'INTRO_PARTS','orientation':ori,'variant':variant,'media_paths':paths,'motion_placements':motions_for(item['content_id'],ori,variant),'vo_note':'SOURCE_REPLACEMENT is an alternative to VO; replace the SR slot with chosen VO later when required.'})
    for item in plan.get('verticals',[]):
        path=Path(cfg['_paths']['vertical'])/item['content_id']/'MEDIA'/f"{item['content_id']}_VERTICAL_SOURCE.mp4"
        fragments=fragment_paths(path.parent/'FRAGMENTS')
        if item.get('status')=='PASS' and (fragments or path.is_file()): resolved.append({'timeline_name':f"{item['content_id']}_VERTICAL_ABRAXAS_{revision}",'content_id':item['content_id'],'type':'CONTENT','orientation':'VERTICAL','variant':'SOURCE','media_paths':fragments or [str(path)],'motion_placements':motions_for(item['content_id'],'VERTICAL','SOURCE')})
    for item in plan.get('horizontals',[]):
        ...
        if item.get('status')=='PASS' and path.is_file(): resolved.append({'timeline_name':f"{item['content_id']}_HORIZONTAL_ABRAXAS_{revision}",'content_id':item['content_id'],'type':'CONTENT_LONGFORM','orientation':'HORIZONTAL','variant':'SOURCE','media_paths':[str(path)],'motion_placements':motions_for(item['content_id'],'HORIZONTAL','SOURCE'),'horizontal_policy':'COMPLETE_VIDEO_PLUS_COHERENT_SECTIONS__MAX_2_MOTIONS_PER_ROLLING_MINUTE'})
    m['resolved_timelines']=resolved
    m['schema_version']='abraxas.davinci-handoff.v4.2'
    m['motion_plan_path']=str(motion_plan_path(cfg))
    m['fps']=cfg.get('davinci',{}).get('fps',30)
    m['behavior']={'existing_timeline':'SKIP_TO_PROTECT_MANUAL_EDITS','missing_motion':'ADD_MARKER_AND_CONTINUE','motion_audio':'NEVER_IMPORT','subtitle_track':'V5_RESERVED'}
    target=Path(cfg['_paths']['davinci'])/'DAVINCI_HANDOFF.json'; write_json_atomic(target,m)
    # Copy a context-zero Workspace Console conformer with markers and motion tracks.
    template=Path(cfg['_package_root'])/'TOOLS'/'davinci_workspace_conform_v4_2.py'; shutil.copy2(template,Path(cfg['_paths']['davinci'])/'DAVINCI_WORKSPACE_CONFORM_V4_2.py')
    launcher=f"import os; p=os.path.expanduser(r'{str(Path(cfg['_paths']['davinci'])/'DAVINCI_WORKSPACE_CONFORM_V4_2.py')}'); exec(compile(open(p,'r',encoding='utf-8').read(),p,'exec'),globals(),globals())"
...
def cmd_status(args):
    ...
    if plan:
        for fam in ('intros','verticals','horizontals'):
            ...
            print(fam.upper(),d)
    motions=read_json(motion_plan_path(cfg),{}) or {}
    if motions:
        refresh_asset_status(motions)
        print('MOTIONS V4.2:',motions.get('counts',{})); print('MOTION ASSETS:',motions.get('asset_counts',{}))
    states=list(Path(cfg['_paths']['state']).glob('*.json')); print('CHECKPOINT FILES:',len(states))

def parser():
    ap=argparse.ArgumentParser(description='ABRAXAS V4.2 HTML-driven cuts, Motion context and DaVinci packages'); ap.add_argument('--config',default=str(package_root()/'PROJECT'/'project_config.json'))
    ...
    sub.add_parser('motions').set_defaults(fn=cmd_motions)
    p=sub.add_parser('motion-previews'); p.add_argument('--dry-run',action='store_true'); p.set_defaults(fn=cmd_motion_previews)
    p=sub.add_parser('motion-verify'); p.add_argument('--strict-assets',action='store_true'); p.set_defaults(fn=cmd_motion_verify)
    p=sub.add_parser('html-packages'); p.add_argument('--structure-only',action='store_true'); p.add_argument('--dry-run',action='store_true'); p.set_defaults(fn=cmd_html_packages)
