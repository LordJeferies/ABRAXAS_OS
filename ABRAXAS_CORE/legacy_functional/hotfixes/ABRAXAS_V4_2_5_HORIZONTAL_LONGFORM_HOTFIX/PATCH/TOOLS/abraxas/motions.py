...
HORIZONTAL_MIN_MOTION_SECONDS = 4.0
HORIZONTAL_MAX_MOTION_SECONDS = 9.0
HORIZONTAL_MAX_MOTIONS_PER_ROLLING_MINUTE = 2
...
def _common_prompt_header(motion_id, placement, treatment):
    ...
    return "\n".join(
        [
            "ABRAXAS V4.2 · PROMPT DE IMAGEN PRODUCTION-READY",
...
def _base_placement(content_id, family, orientation, variant, beat, treatment, selection):
    ...
    return {
        ...
        "timeline_out": None,
        "duration": duration,
        "selection_source": "DETERMINISTIC_V4_2",
        **selection,
    }
...
def compile_motion_plan(bundle, part_plan, resolutions, overrides=None):
    ...
    for family, orientation in (("verticals", "vertical"), ("horizontals", "horizontal")):
        for item in content.get(family, []):
            ...
            if orientation == "horizontal":
                # Long-form horizontals are not physical 4–9 second fragments.
                # Only editorially selected visual opportunities become Motion
                # windows; the complete video and its coherent source sections
                # remain intact.
                beats = [beat for beat in beats if beat.get("beat_id") in opportunity_by_beat]
            for beat in beats:
    ...
    horizontal_raw = [x for x in placements if x.get("content_family") == "horizontals"]
    short_form_raw = [x for x in placements if x.get("content_family") != "horizontals"]
    short_form = _compile_balanced_fragments(short_form_raw, overrides)
    horizontal, horizontal_audit = compile_horizontal_motion_windows(horizontal_raw)
    placements = short_form + horizontal
    durations = [float(x.get("duration") or 0) for x in placements]
    return {
        "schema_version": "abraxas.motion-plan.v4.2",
        ...
        "delivery_contracts": {
            "verticals_and_intros": {
                "physical_parts": True,
                "minimum_seconds": 4.0,
                "maximum_seconds": 9.0,
                "target_average_seconds": 8.0,
            },
            "horizontals": {
                "physical_parts": False,
                "video_structure": "COMPLETE_VIDEO_PLUS_COHERENT_SOURCE_SECTIONS",
                "motion_window_minimum_seconds": HORIZONTAL_MIN_MOTION_SECONDS,
                "motion_window_maximum_seconds": HORIZONTAL_MAX_MOTION_SECONDS,
                "maximum_motions_per_rolling_minute": HORIZONTAL_MAX_MOTIONS_PER_ROLLING_MINUTE,
            },
        },
        "horizontal_policy_audit": horizontal_audit,
        "placements": placements,
    }


def compile_horizontal_motion_windows(raw_placements):
    """Keep only sparse Motion windows for long-form horizontal videos.

    Horizontal source video is never divided here. A selected editorial visual
    opportunity may create one 4–9 second overlay window. The rolling limiter
    guarantees that no 60 second span contains more than two Motion starts.
    Unmapped, presenter-only and shorter-than-four-second opportunities remain
    source and are recorded in the audit rather than being stretched.
    """
    grouped = defaultdict(list)
    for placement in raw_placements:
        grouped[(placement["content_id"], placement.get("assembly_variant", "SOURCE"))].append(placement)

    result = []
    audit = {"selected": 0, "omitted": 0, "by_reason": {}, "videos": {}}

    def omit(video_audit, placement, reason):
        audit["omitted"] += 1
        audit["by_reason"][reason] = audit["by_reason"].get(reason, 0) + 1
        video_audit["omitted"].append({"beat_id": placement.get("beat_id"), "reason": reason})

    for (content_id, variant), items in grouped.items():
        candidates = sorted(
            items,
            key=lambda x: (
                float(x.get("timeline_in")) if x.get("timeline_in") is not None else float("inf"),
                str(x.get("beat_id") or ""),
            ),
        )
        selected_starts = []
        video_audit = {"selected": [], "omitted": []}
        for placement in candidates:
            if placement.get("motion_primary") == "M0":
                omit(video_audit, placement, "KEEP_SOURCE")
                continue
            if placement.get("timeline_in") is None or placement.get("timeline_out") is None:
                omit(video_audit, placement, "UNMAPPED_TIMING")
                continue

            start = float(placement["timeline_in"])
            original_duration = max(0.0, float(placement["timeline_out"]) - start)
            if original_duration < HORIZONTAL_MIN_MOTION_SECONDS - 1e-6:
                omit(video_audit, placement, "WINDOW_UNDER_4_SECONDS_KEEP_SOURCE")
                continue

            recent = [value for value in selected_starts if start - value < 60.0 - 1e-6]
            if len(recent) >= HORIZONTAL_MAX_MOTIONS_PER_ROLLING_MINUTE:
                omit(video_audit, placement, "DENSITY_CAP_2_PER_ROLLING_MINUTE")
                continue

            duration = min(HORIZONTAL_MAX_MOTION_SECONDS, original_duration)
            selected = dict(placement)
            selected["editorial_source_in"] = placement.get("source_in")
            selected["editorial_source_out"] = placement.get("source_out")
            selected["editorial_duration"] = round(original_duration, 3)
            selected["timeline_out"] = round(start + duration, 3)
            selected["duration"] = round(duration, 3)
            selected["planned_seconds"] = round(duration, 3)
            if selected.get("source_in") is not None:
                selected["source_out"] = round(float(selected["source_in"]) + duration, 3)
            selected["timing_status"] = "HORIZONTAL_MOTION_WINDOW_4_TO_9"
            selected["selection_source"] = "HTML_VISUAL_OPPORTUNITY_LONGFORM_POLICY"
            selected["readiness"] = "READY_FOR_IMAGE_GENERATION"
            selected["delivery_role"] = "MOTION_INSERT_ONLY__DO_NOT_FRAGMENT_HORIZONTAL_VIDEO"
            selected["fragment_contract"] = "MOTION_WINDOW_ONLY:4.000<=duration<=9.000"
            selected["horizontal_motion_density_policy"] = "MAX_2_STARTS_IN_ANY_ROLLING_60_SECONDS"
            result.append(selected)
            selected_starts.append(start)
            audit["selected"] += 1
            video_audit["selected"].append(
                {
                    "beat_id": selected.get("beat_id"),
                    "timeline_in": selected.get("timeline_in"),
                    "timeline_out": selected.get("timeline_out"),
                    "duration": selected.get("duration"),
                }
            )
        audit["videos"][content_id] = video_audit
    return result, audit
    ...
    for (family, content_id, orientation, variant), items in grouped.items():
        ...
        for index, (start, end) in enumerate(ranges, 1):
            ...
            fragment = {
                ...
                "timeline_out": end,
                "duration": duration,
                "selection_source": "BALANCED_FRAGMENT_V4_2",
                "readiness": "KEEP_SOURCE" if selection["motion_primary"] == "M0" else "READY_FOR_IMAGE_GENERATION",
...
def _animation_prompt(placement, treatment):
    ...
    return "\n\n".join(
        [
            "ABRAXAS V4.2 · PROMPT FINAL DE ANIMACIÓN",
...
def write_motion_tree(plan, output_root):
    ...
    motions_root = ensure_dir(output_root / "09_MOTIONS_V4_2")
    unique = {}
    grouped = defaultdict(list)
    ...
    for (family, content_id, orientation), placements in grouped.items():
        ...
        lines = [
            "PROMPT MAESTRO ABRAXAS V4.2 · GENERAR TODAS LAS IMÁGENES DE ESTE VIDEO",
    ...
    manifest = output_root / "00_MANIFEST" / "MOTION_PLAN_V4_2.json"
    write_json_atomic(manifest, plan)
    write_json_atomic(motions_root / "MOTION_QUEUE_V4_2.json", plan)
    return manifest
...
def build_motion_previews(cfg, plan, dry_run=False):
    ...
    return {"schema_version": "abraxas.motion-preview-report.v4.2", "results": results, "plan": plan}
