...
def collect_microtrims(bundle,resolutions):
    out=[]
    # V4.2.1 resolves the physical 4–9 s delivery parts, not every legacy
    # semantic beat. This gives every folder an approved range while avoiding
    # duplicate ASR work on internal editorial atoms.
    c=bundle['content']
    for fam,ori in [('verticals','vertical'),('horizontals','horizontal')]:
        for item in c.get(fam,[]):
            if fam == 'horizontals':
                # A horizontal is long-form. Resolve only the sparse visual
                # opportunities that may receive a Motion; never resolve or
                # render every 4–9 second transcript atom as a physical clip.
                selected={x.get('beat_id') for x in item.get('visual_opportunities',[]) if x.get('beat_id')}
                physical=[x for x in item.get('beats',[]) if x.get('beat_id') in selected]
            else:
                physical=item.get('delivery_parts_4_9') or item.get('beats',[])
            for beat in physical:
    ...
    for intro in bundle['intro'].get('intros',[]):
        strict=intro.get('delivery_variants_4_9') or {}
        if strict:
            beats=[beat for sequence in strict.values() for beat in sequence]
        else:
