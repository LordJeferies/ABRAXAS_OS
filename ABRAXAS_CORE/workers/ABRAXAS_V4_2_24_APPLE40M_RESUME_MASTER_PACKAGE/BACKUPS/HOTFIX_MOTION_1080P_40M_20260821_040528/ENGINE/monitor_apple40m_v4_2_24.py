...
def valid_reference(path: Path) -> bool:
    ...
    try:
        ...
        return (
            value.get("profile") == "APPLE_VT_H264_40M_V1"
...
print("\nABRAXAS V4.2.24 · APPLE VIDEOTOOLBOX · 40M")
print(f"OUTPUT: {root}")
...
for path in completed_initial:
    sequence += 1
    kind = "REF40M" if path in refs_initial else "VIDEO"
...
while True:
    ...
    if newly_finished:
        ...
        for path in newly_finished:
            sequence += 1
            kind = "REF40M" if path in refs else "VIDEO"
            print(f"{sequence:03d} ✅ {kind:<6} {path.relative_to(root)}")
            printed.add(path)
    ...
    if jobs:
        ...
        source = "FFMPEG" if current is not None else "INICIANDO"
        name = short_name(root, job["final"])
        status = (
            f"{symbol} {name} | {percent:5.1f}% {source} | ETA {human_time(eta)} | {activity} | "
        ...
        status = (
            ...
            f"{global_percent:.1f}% · faltan {remaining} | REF40M {len(refs)}/{TOTAL_REFS} · VIDEO {len(videos)}/{TOTAL_VIDEOS}"
        )

    columns = shutil.get_terminal_size((180, 30)).columns
    print("\r\033[2K" + status[: max(40, columns - 1)], end="", flush=True)
    if done >= TOTAL:
        print("\n\n✅ PRODUCCIÓN ABRAXAS 40M COMPLETA")
        break
    time.sleep(1)
