...
def prompt_bundle(content_id: str, role: str, candidate: Candidate, motion: str) -> dict[str, str]:
    ...
    image = (
        f"ABRAXAS {VERSION} · {content_id} · {role} · {motion} {name}. Formato {orientation}. "
    ...
    animation = (
        f"ABRAXAS {VERSION} · Animar {motion} {name} para {content_id}/{role}. Duración exacta {candidate.duration:.3f}s; formato {orientation}. "
    ...
    for folder_name, family in (("03_VERTICALS", mapping["verticals"]), ("04_HORIZONTALS", mapping["horizontals"])):
        for content_id, item in family.items():
            ...
            for motion in item["motions"]:
                ...
                write_text(folder / "20_MOTION_FINAL" / "COLOCAR_AQUI_MOTION_FINAL.txt", "Guardar aquí MOTION_FINAL.mov o MOTION_FINAL.mp4, sin audio y con la duración exacta.")
...
def probe_reference(path: Path, ffprobe: str, expected_duration: float) -> tuple[bool, dict[str, Any]]:
    if not path.is_file() or path.stat().st_size < 1024:
        return False, {"reason": "MISSING_OR_TOO_SMALL"}
    command = [
        ffprobe, "-v", "error", "-show_entries",
        "stream=codec_type,codec_name,pix_fmt,sample_rate,channels:format=duration,size,bit_rate",
        "-of", "json", str(path),
    ]
    try:
...
def reference_command(ffmpeg: str, master: Path, motion: dict[str, Any], partial: Path, progress: Path, hwdecode: bool) -> list[str]:
            ...
            for motion in item["motions"]:
                ...
                if sidecar.is_file():
                    try:
                        ...
                        sidecar_value = {}
                profile_is_current = (
                    sidecar_value.get("profile") == "APPLE_VT_H264_40M_V1"
                ...
                valid, probe = probe_reference(target, ffprobe, motion["duration_seconds"])
                ...
                if valid:
                    ...
                    continue
                if target.exists():
                    invalid = folder / "30_EXPORTS" / f"SOURCE_REFERENCE_INVALID_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.mp4"
                ...
                command = reference_command(ffmpeg, master, motion, partial, progress, hwdecode=True)
                returncode, detail = run_reference_encode(command, log_path, timeout_seconds)
                if returncode != 0:
                    partial.unlink(missing_ok=True)
                    command = reference_command(ffmpeg, master, motion, partial, progress, hwdecode=False)
                    returncode, detail = run_reference_encode(command, log_path, timeout_seconds)
                if returncode == 0:
                    valid, probe = probe_reference(partial, ffprobe, motion["duration_seconds"])
                else:
                    valid, probe = False, {"reason": detail}
                if valid:
                    os.replace(partial, target)
                    write_json(sidecar, {
                        "schema_version": "abraxas.reference-profile.v4.2.24.apple40m-r1",
                        "profile": "APPLE_VT_H264_40M_V1",
                    ...
                    status = "CREATED_APPLE_VT_40M"
                else:
                    partial.unlink(missing_ok=True)
                    status = "FAILED_VALIDATION_OR_TIMEOUT"
                counts[status] = counts.get(status, 0) + 1
                results.append({"content_id": content_id, "sequence": motion["sequence"], "status": status, "target": str(target), "probe": probe, "detail": detail, "profile": "APPLE_VT_H264_40M_V1"})
                time.sleep(0.35)
    return {
        "schema_version": "abraxas.reference-render-report.v4.2.24.apple40m-r1",
        "generated_at": utc_now(),
        "profile": "APPLE_VT_H264_40M_V1",
