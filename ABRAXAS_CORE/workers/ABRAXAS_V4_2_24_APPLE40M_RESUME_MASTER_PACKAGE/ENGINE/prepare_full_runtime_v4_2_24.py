...
def main() -> int:
    ...
    data["render"].update({
        ...
        "hardware_encode_slots": 1,
        "encode_timeout_seconds": 1800,
        "lock_wait_seconds": 7200,
        "atomic_output": "PARTIAL_VALIDATE_OS_REPLACE",
    })
...
"""
    ...
    render_path = runtime / "TOOLS" / "abraxas" / "render.py"
    render_raw = render_path.read_text(encoding="utf-8")
    old_encode_tail = "'-movflags','+faststart',str(partial)]"
    new_encode_tail = "'-movflags','+faststart','-stats_period','1','-progress',str(partial.with_suffix(partial.suffix+'.progress.txt')),'-nostats',str(partial)]"
    if old_encode_tail in render_raw:
        render_raw = render_raw.replace(old_encode_tail, new_encode_tail)
    old_concat = "[ffmpeg,'-hide_banner','-y','-f','concat','-safe','0','-i',str(list_file),'-c','copy','-movflags','+faststart',str(partial)]"
    if old_concat in render_raw:
        render_raw = render_raw.replace(old_concat, "[ffmpeg,'-hide_banner','-y','-f','concat','-safe','0','-i',str(list_file),'-c','copy','-movflags','+faststart','-stats_period','1','-progress',str(partial.with_suffix(partial.suffix+'.progress.txt')),'-nostats',str(partial)]")
    render_path.write_text(render_raw, encoding="utf-8")
    runtime_path = runtime / "TOOLS" / "abraxas" / "runtime.py"
    runtime_raw = runtime_path.read_text(encoding="utf-8")
    runtime_raw = runtime_raw.replace(
        "subprocess.run(cmd,capture_output=True,text=True)",
        "subprocess.run(cmd,capture_output=True,text=True,timeout=cfg['render'].get('encode_timeout_seconds',1800))",
    )
    runtime_path.write_text(runtime_raw, encoding="utf-8")
    print(config_path)
    return 0
