...
def write_motion_packages(output_root: Path, mapping: dict[str, Any]) -> Path:
    ...
    if package_root.exists():
        backup = output_root / "00_CONTROL" / "BACKUPS" / f"AI_SELECTED_V4_2_24_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        backup.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(package_root), str(backup))
...
def render_reference_clips(mapping: dict[str, Any], package_root: Path, vertical_master: Path | None, horizontal_master: Path | None, ffmpeg: str) -> dict[str, Any]:
    results = []
    for folder_name, family, master in (("03_VERTICALS", mapping["verticals"], vertical_master), ("04_HORIZONTALS", mapping["horizontals"], horizontal_master)):
        ...
        for content_id, item in family.items():
            for motion in item["motions"]:
                ...
                command = [ffmpeg, "-hide_banner", "-loglevel", "error", "-y", "-ss", str(parse_timecode(motion["source_start"])), "-i", str(master), "-t", f"{motion['duration_seconds']:.3f}", "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-c:a", "aac", "-b:a", "160k", str(target)]
                proc = subprocess.run(command, capture_output=True, text=True)
                results.append({"content_id": content_id, "sequence": motion["sequence"], "status": "CREATED" if proc.returncode == 0 else "FAILED", "target": str(target), "error": proc.stderr.strip()[-600:] if proc.returncode else None})
    return {"schema_version": "abraxas.reference-render-report.v4.2.24", "generated_at": utc_now(), "results": results}
...
def build(args: argparse.Namespace) -> int:
    ...
    if args.render_references:
        render_report = render_reference_clips(mapping, package_root, Path(args.vertical_master).expanduser().resolve() if args.vertical_master else None, Path(args.horizontal_master).expanduser().resolve() if args.horizontal_master else None, args.ffmpeg)
