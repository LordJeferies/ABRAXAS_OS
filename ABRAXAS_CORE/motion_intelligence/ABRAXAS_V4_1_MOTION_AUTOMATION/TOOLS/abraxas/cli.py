...
def hydrate_cfg(cfg):
    ...
    paths={
      ...
      'visual_queue':str(out/'09_VISUAL_QUEUE'),'motions':str(out/'09_MOTIONS_V4_1'),'davinci':str(out/'10_DAVINCI_HANDOFF'),'final':str(out/'11_FINAL'),
...
def motion_plan_path(cfg): return Path(cfg['_paths']['manifest'])/'MOTION_PLAN_V4_1.json'
...
def cmd_setup(args):
    ...
    if args.joc55:
        ...
        dout=str(Path.home()/"Desktop/Joc podcast next ep 55/JOC55_ABRAXAS_V4_1_OUTPUT")
        pid='JOC55_AMANDA'
    else:
        content=args.content or ''; intro=args.intro or ''; dv=args.vertical or ''; dh=args.horizontal or ''; dout=args.output or str(Path.home()/'Desktop/ABRAXAS_V4_1_OUTPUT'); pid=args.project_id or 'PROJECT'
...
def cmd_plan(args):
    ...
    if cfg.get('motion',{}).get('enabled',True): summary['motions_v4_1']=motion_plan['counts']
...
def cmd_motion_previews(args):
    ...
    if not plan: raise SystemExit('MOTION_PLAN_V4_1 missing; run 08B_BUILD_MOTIONS_AND_PROMPTS.command first')
    report=build_motion_previews(cfg,plan,dry_run=args.dry_run)
    write_json_atomic(motion_plan_path(cfg),report['plan'])
    write_json_atomic(Path(cfg['_paths']['manifest'])/'MOTION_PREVIEW_REPORT_V4_1.json',{k:v for k,v in report.items() if k!='plan'})
    counts={}
...
def cmd_motion_verify(args):
    cfg=load_cfg(args.config); plan=read_json(motion_plan_path(cfg))
    if not plan: raise SystemExit('MOTION_PLAN_V4_1 missing')
    ...
    write_json_atomic(Path(cfg['_paths']['manifest'])/'MOTION_VERIFY_REPORT_V4_1.json',report)
...
def cmd_davinci(args):
    ...
    revision=cfg.get('davinci',{}).get('revision','V4_1')
    ...
    for item in plan.get('horizontals',[]):
        ...
        fragments=fragment_paths(path.parent/'FRAGMENTS')
        if item.get('status')=='PASS' and (fragments or path.is_file()): resolved.append({'timeline_name':f"{item['content_id']}_HORIZONTAL_ABRAXAS_{revision}",'content_id':item['content_id'],'type':'CONTENT','orientation':'HORIZONTAL','variant':'SOURCE','media_paths':fragments or [str(path)],'motion_placements':motions_for(item['content_id'],'HORIZONTAL','SOURCE')})
    m['resolved_timelines']=resolved
    m['schema_version']='abraxas.davinci-handoff.v4.1'
    ...
    template=Path(cfg['_package_root'])/'TOOLS'/'davinci_workspace_conform_v4_1.py'; shutil.copy2(template,Path(cfg['_paths']['davinci'])/'DAVINCI_WORKSPACE_CONFORM_V4_1.py')
    launcher=f"import os; p=os.path.expanduser(r'{str(Path(cfg['_paths']['davinci'])/'DAVINCI_WORKSPACE_CONFORM_V4_1.py')}'); exec(compile(open(p,'r',encoding='utf-8').read(),p,'exec'),globals(),globals())"
...
def cmd_status(args):
    ...
    if motions:
        ...
        print('MOTIONS V4.1:',motions.get('counts',{})); print('MOTION ASSETS:',motions.get('asset_counts',{}))
    states=list(Path(cfg['_paths']['state']).glob('*.json')); print('CHECKPOINT FILES:',len(states))

def parser():
    ap=argparse.ArgumentParser(description='ABRAXAS V4.1 executable automation with Motion conform'); ap.add_argument('--config',default=str(package_root()/'PROJECT'/'project_config.json'))
