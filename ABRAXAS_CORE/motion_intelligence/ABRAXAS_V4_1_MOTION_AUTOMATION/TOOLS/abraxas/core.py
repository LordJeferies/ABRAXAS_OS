...
def balanced_fragment_ranges(total_seconds, minimum=4.0, maximum=9.0, target=8.0):
    """Return contiguous, balanced ranges with no short tail.

    For any program at least ``minimum`` seconds long, every range is within
    [minimum, maximum]. The number of ranges is selected near ``target`` and
    the remainder is distributed across all ranges instead of creating a
    one-second final fragment.
    """
    total=float(total_seconds or 0)
    if total <= 0:
        return []
    if total < minimum:
        raise ValueError(f'PROGRAM_TOO_SHORT_FOR_FRAGMENT_CONTRACT:{total:.3f}s')
    min_count=max(1,int(math.ceil(total/maximum-1e-9)))
    max_count=max(1,int(math.floor(total/minimum+1e-9)))
    if min_count > max_count:
        raise ValueError(f'NO_VALID_FRAGMENT_PARTITION:{total:.3f}s')
    count=min(range(min_count,max_count+1),key=lambda n:(abs(total/n-target),n))
    base=total/count
    ranges=[]; cursor=0.0
    for index in range(count):
        end=total if index==count-1 else base*(index+1)
